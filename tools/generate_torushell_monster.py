...
SOURCE = ROOT / "tools" / "sources" / "torushell-base-idle-generated.png"
OUTPUT = ROOT / "assets" / "images" / "monster-tamer" / "monsters" / "torushell-battle-idle.png"
FRAME_SIZE = 256
FRAME_COUNT = 4
...
def main() -> None:
    ...
    for left, right in columns:
        ...
        if bbox is None:
            raise RuntimeError("Generated Torushell frame is empty")
        frames.append(frame.crop(bbox))

    max_width = max(frame.width for frame in frames)
    max_height = max(frame.height for frame in frames)
    scale = min(244 / max_width, 238 / max_height)
